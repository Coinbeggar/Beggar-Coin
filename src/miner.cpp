...
{
    // double check that there are no double spent zBGGR spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zBGGR serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zBGGR spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zBGGR check to not include duplicated serials in the same block.
