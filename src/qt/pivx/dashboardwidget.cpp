    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of BGGR earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 BGGR");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("BGGR")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair BGGR, zBGGR
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(125,206,160));
	set1->setColor(QColor(84,153,199));
