...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("beggar client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("beggar"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("beggar"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
