...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected BGGR and zBGGR supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zBGGRsupply']:
                assert_equal(node['zBGGRsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check BGGR and zBGGR supply at the beginning
        # ------------------------------------------
        # zBGGR supply: 2 coins for each denomination
        ...
        # BGGR supply: block rewards minus burned fees for minting
        ...
        # Verify that BGGR and zBGGR supplies were properly updated after the spends and reorgs
        self.log.info("Check BGGR and zBGGR supply...")
